    ...
    <header className="bg-white border-b border-gray-200 shadow-sm sticky top-0 z-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          {/* Search Bar - Only show on dashboard page */}
          ...
          {user && (
            <div className="flex items-center space-x-4">
              ...
              <button
                ...
                className="relative flex items-center space-x-1 px-3 py-2 rounded-lg text-gray-700 hover:text-black hover:bg-gray-100 transition-colors duration-200"
              >
                <ShoppingCart className="h-5 w-5" />
                <span>Cart</span>
                {getTotalItems() > 0 && (
                  <span className="absolute -top-2 -right-2 bg-black text-white text-xs rounded-full h-6 w-6 flex items-center justify-center font-medium">
                    {getTotalItems()}
                  </span>
                )}
              </button>
